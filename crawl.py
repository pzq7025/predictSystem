...
# 解析网页
def parser(html):
    # html = html.decode(encoding='gbk', errors="ignore")
    ...
    for i in salaries:
        ...
        if len(result_s) == 1:
            unity_salaries.append(f"{result_s[0] * 24 * 3}")
            continue
        ...
        if first == '元' and end == '小时':
            new_low, new_high = low * 24 * 3, high * 24 * 3
    ...
    # 二级网页
    second_urls = match_html.xpath('/html/body/div[4]/div[2]/div[1]/div[2]/div/p[1]/span[1]/a/@href')
    # request_occupy
    request_occupy = []  # 需求值
    # print(tasks)
    for i in second_urls:
        response_1 = requests.get(i, headers=header).content
        result = parser_second(response_1)
        request_occupy.append(result)
    request_occupies = ["无" if i == '' else i.replace(' ', '/') for i in request_occupy]  # 需求值
    # print(request_occupy)
    ...
    # dataframe的使用
    for i in list(zip(occupations, company_names, locations, unity_salaries, dates, educations, experiences, company_types, company_sizes, request_occupies, contents)):
...
def parser_second(html):
    html = html.decode(encoding='gbk', errors="ignore")
    match_html = etree.HTML(html)
    ability = match_html.xpath('/html/body/div[3]/div[2]/div[3]/div[1]/div/div[1]/p[1]/a//text()')
    return ' '.join(ability)


# 处理二级网页
# async def download_second(sem, url):
#     async with aiohttp.ClientSession() as session:
#         try:
#             html = await fetch(sem, session, url)
#             return await parser_second(html)
#         except Exception as err:
#             print(err)
...
if __name__ == '__main__':
    ...
    # response11 = requests.get("https://jobs.51job.com/p4/", headers=header).content
    # parser(response11)
